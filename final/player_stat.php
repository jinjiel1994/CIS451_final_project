...
<html>
    ...
    <body bgcolor='white'>
        ...
        $query1="SELECT p1.Player_name, IF(ISNULL(pp.GP),0, pp.GP) AS GP, IF(ISNULL(pp.MIN),0, pp.MIN) AS MIN, <br>
                ...
                WHERE p1.Player_name ='".$player_name."';";


        $query2="SELECT p1.Player_name, IF(ISNULL(pp.GP),0, pp.GP) AS GP, IF(ISNULL(pp.MIN),0, pp.MIN) AS MIN,
   ...
   $result->execute();
    ...
    {
        print "<tr>";
        print "<td>$row[Player_name]</td> <td>$row[GP]</td> <td>$row[MIN]</td> <td>$row[PTS]</td> <td>$row[REB]</td> <td>$row[AST]</td> <td>$row[STL]</td> <td>$row[BLK]</td>  <td>$row[TOV]</td> <td>$row[FG_PCT]</td> <td>$row[FG3_PCT]</td> <td>$row[FT_PCT]</td> ";
        print "</tr>";
    }
    print "</table>";
    print "</pre>";
